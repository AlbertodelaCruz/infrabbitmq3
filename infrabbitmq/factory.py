...
def json_serializer():
    return factory.Factory.instance('json_serializer', lambda: serializers.JsonSerializer())


def pickle_serializer():
    return factory.Factory.instance('pickle_serializer', lambda: serializers.PickleSerializer())


def json_or_pickle_serializer():
    return factory.Factory.instance('json_or_pickle_serializer', lambda: serializers.JsonOrPickleSerializer())


def event_publisher(exchange='events', broker_uri=None):
    return factory.Factory.instance('event_publisher_%s_%s' % (exchange, broker_uri),
...
def event_publisher_pickle_serializer(exchange='events', broker_uri=None):
    return factory.Factory.instance('event_publisher_pickle_serializer_%s_%s' % (exchange, broker_uri),
        lambda: rabbitmq.EventPublisher(
...
def event_publisher_json_serializer(exchange='events', broker_uri=None):
    return factory.Factory.instance('event_publisher_json_serializer{}_{}'.format(exchange, broker_uri),
